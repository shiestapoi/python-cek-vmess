...
==============================================
A comprehensive tool to decode, analyze, and generate reports for VMess and VLESS proxy configurations.

Features:
- Decode VMess (base64-json and URI-style) and VLESS configs
- IP Geolocation lookup with fallback providers
- Connectivity testing via Xray core
...
# Default API keys for ipgeolocation.io (can be overridden via .env)
IPGEOLOCATION_KEYS = [
...
def parse_bool_like(value: Any) -> bool:
    """Parse various boolean-like values to Python bool."""
...
class ApiKeyPool:
    """Thread-safe pool for managing API keys with disable capability."""
    ...
    def disable(self, key: str) -> None:
        """Disable a specific API key (e.g., when quota exceeded)."""
        with self._lock:
            if key in self._state:
                self._state[key] = False

    def has_active(self) -> bool:
        """Check if any API key is still active."""
        with self._lock:
...
def extract_vmess_tokens(text: str) -> List[str]:
    """Extract all VMess URLs from text."""
    tokens: List[str] = []
    for match in VMESS_PATTERN.finditer(text):
        raw = match.group(0)
        clean = sanitize_vmess_token(raw)
        if clean:
            tokens.append(clean)
    return tokens
...
def extract_vless_tokens(text: str) -> List[str]:
    """Extract all VLESS URLs from text."""
    tokens: List[str] = []
    for match in VLESS_PATTERN.finditer(text):
        raw = match.group(0)
        clean = sanitize_vless_token(raw)
        if clean:
            tokens.append(clean)
    return tokens


def sanitize_vmess_token(token: str) -> str:
    """Clean and validate VMess token."""
...
def sanitize_vless_token(token: str) -> str:
    """Clean and validate VLESS token."""
    ...
    # VLESS is always URI-style, keep URL-safe characters
    url_chars = re.match(r"^[A-Za-z0-9\-._~:/?#\[\]@!$&'()*+,;=%]+", body)
    if url_chars:
        return "vless://" + url_chars.group(0)
    return ""
...
def parse_vmess(token: str) -> Dict[str, Any]:
    """Parse VMess token and extract configuration details."""
    result: Dict[str, Any] = {
        "type": "vmess",
        "original": token,
        "format": "unknown",
...
def parse_vless(token: str) -> Dict[str, Any]:
    """Parse VLESS token and extract configuration details."""
    result: Dict[str, Any] = {
        "type": "vless",
        "original": token,
        "format": "uri-style",
    ...
    if parsed.hostname:
        ...
        # Check security parameter for TLS
        security = ""
        if "security" in query and query["security"]:
            security = str(query["security"][0]).strip().lower()
        result["tls_enabled"] = security in {"tls", "xtls", "reality"}
        
        # Store decoded info for connectivity check
        result["decoded"] = {
            "id": parsed.username or "",
            "address": parsed.hostname,
            "port": parsed.port or 0,
            "security": security,
            "flow": query.get("flow", [""])[0] if "flow" in query else "",
            "type": query.get("type", ["tcp"])[0] if "type" in query else "tcp",
            "host": query.get("host", [""])[0] if "host" in query else "",
            "path": query.get("path", ["/"])[0] if "path" in query else "/",
            "sni": query.get("sni", [""])[0] if "sni" in query else "",
        }
        return result

    result["status"] = "error"
    result["error"] = "Unable to parse vless URI"
    return result


def derive_endpoint_fields(row: Dict[str, Any]) -> None:
    """Derive endpoint fields from original token if missing."""
    if row.get("endpoint_port") and row.get("add") and ("tls_enabled" in row):
        return

    token = str(row.get("original", "")).strip()
    if not token:
        return

    if token.startswith("vmess://"):
        parsed = parse_vmess(token)
    elif token.startswith("vless://"):
...
def unknown_info(source: str = "none") -> Dict[str, str]:
    """Return unknown geolocation info."""
    return {
        "country": "Unknown",
        "city": "Unknown",
        "isp": "Unknown",
...
def run_curl_text(
    ...
    """Run curl command and return text response."""
...
def check_connectivity_endpoint(
    add: str, resolved_ip: str, port: int, timeout: float
) -> Tuple[str, str]:
    """Check TCP connectivity to endpoint."""
...
def check_vmess_connectivity_via_xray(
    ...
    """Check VMess connectivity using Xray core."""
...
def check_vless_connectivity_via_xray(
    ...
    """Check VLESS connectivity using Xray core."""
    xray_bin = shutil.which("xray")
    if not xray_bin:
        return "failed", "xray core not found in PATH (need 'xray')"
    ...
    decoded = parsed.get("decoded")
    if not isinstance(decoded, dict):
        return "failed", "Decoded VLESS info required for xray mode"

    vless_id = str(decoded.get("id", "")).strip()
    if not vless_id:
        return "failed", "Missing vless id"

    net = str(decoded.get("type", "tcp") or "tcp").strip().lower()
    security = str(decoded.get("security", "") or "").strip().lower()
    host = str(decoded.get("host", "") or "").strip()
    path = str(decoded.get("path", "") or "").strip() or "/"
    sni = str(decoded.get("sni", "") or "").strip()
    flow = str(decoded.get("flow", "") or "").strip()

    stream_settings: Dict[str, Any] = {"network": net}
    
    # Handle different security types
    if security in ["tls", "xtls", "reality"]:
        stream_settings["security"] = security if security != "reality" else "reality"
        if security == "reality":
    ...
    for _ in range(3):
        ...
        user_settings: Dict[str, Any] = {"id": vless_id}
        if flow:
...
def check_proxy_connectivity(
    proxy_type: str, token: str, add: str, resolved_ip: str, port: int, timeout: float
) -> Tuple[str, str]:
    """Check connectivity based on proxy type."""
    if proxy_type == "vmess":
        ...
        return "failed", f"Unknown proxy type: {proxy_type}"
...
def should_disable_key(data: Dict[str, Any]) -> bool:
    """Check if API key should be disabled based on response."""
...
def parse_ipgeolocation_response(data: Dict[str, Any]) -> Optional[Dict[str, str]]:
    """Parse ipgeolocation.io API response."""
...
def parse_ipapi_response(data: Dict[str, Any]) -> Optional[Dict[str, str]]:
    """Parse ipapi.co free API response."""
    # ipapi.co returns error field if something went wrong
    if "error" in data and data["error"]:
        return None
    
    # Check if we have the required fields
    if "country_name" not in data and "country" not in data:
        return None

    country = str(data.get("country_name") or data.get("country") or "Unknown")
    city = str(data.get("city") or "Unknown")
    isp = str(data.get("org") or data.get("asn_org") or "Unknown")
    asn = str(data.get("asn") or "")
    
    # Build services string
    parts = [p for p in [asn] if p]
    services = " | ".join(parts) if parts else "Unknown"
...
def lookup_ip_info(
    ip: str, key_pool: ApiKeyPool, timeout: float, use_free_api: bool = True
) -> Dict[str, str]:
    """
    Lookup IP geolocation information.
    
    Strategy:
    1. Try ipgeolocation.io with API keys (if available)
    2. Fall back to ipapi.co (free, no API key required)
    """
    if not ip:
        return unknown_info("none")

    # Try ipgeolocation.io with API keys first
    ...
    # Fall back to ipapi.co (free API, no key required)
    if use_free_api:
        free_url = f"https://ipapi.co/{urllib.parse.quote(ip)}/json/"
        free_data = run_curl_json(free_url, timeout)
        if free_data:
            parsed = parse_ipapi_response(free_data)
            if parsed:
                return parsed
...
def generate_html(rows: List[Dict[str, Any]], source_file: str, mode: str = MODE_ALL) -> str:
    """Generate interactive HTML report."""
    countries = sorted({row.get("country", "Unknown") for row in rows})
    now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    
    # Determine proxy type label
    type_label = "VMess"
    if mode == MODE_VLESS:
        type_label = "VLESS"
    elif mode == MODE_ALL:
        type_label = "VMess & VLESS"
...
<head>
  ...
  <title>Proxy Checker Report - {type_label}</title>
  <style>
    :root {{
      --bg: #0b111b;
      --bg-alt: #121b2a;
    ...
    * {{ box-sizing: border-box; }}
    ...
    .tag-vmess {{ background: rgba(124, 58, 237, 0.2); border-color: var(--vmess); color: #c4b5fd; }}
    .tag-vless {{ background: rgba(245, 158, 11, 0.2); border-color: var(--vless); color: #fcd34d; }}
    .ok {{ color: var(--good); }}
    .err {{ color: var(--bad); }}
    .stats {{
      display: flex;
...
<body>
  <div class="wrap">
    <section class="hero">
      <h1>{type_label} Decode + IP Intelligence Report</h1>
      ...
      <div class="stats">
        ...
        <div class="chip" id="countVmess">VMess: 0</div>
        <div class="chip" id="countVless">VLESS: 0</div>
      </div>
    </section>

    <section class="controls">
      <input id="search" type="text" placeholder="Search config / IP / ISP / host..." />
  ...
  <div id="detailModal" class="modal" onclick="closeModal()">
    <div class="modal-card" onclick="event.stopPropagation()">
      <div class="modal-head">
        <div class="modal-title">Proxy Config Detail</div>
  ...
  <script>
    ...
    const countVmessEl = document.getElementById('countVmess');
    const countVlessEl = document.getElementById('countVless');
    ...
    function typeClass(t) {{
      return t === 'vless' ? 'tag tag-vless' : 'tag tag-vmess';
    }}

    async function copyConfig(btn) {{
      const raw = btn.getAttribute('data-config') || '';
    ...
    function buildTypeOptions() {{
      const prevValue = typeEl.value || 'ALL';
      const options = ['vmess', 'vless'];
      typeEl.innerHTML = '';
      const allType = document.createElement('option');
      allType.value = 'ALL';
      allType.textContent = 'All Types';
      typeEl.appendChild(allType);
      options.forEach((item) => {{
        const opt = document.createElement('option');
        opt.value = item;
      ...
      typeEl.value = options.includes(prevValue) ? prevValue : 'ALL';
    }}

    function rowMatches(row, q, country, isp, conn, tls, type) {{
    ...
    function openDetail(index) {{
      ...
      const typeLabel = row.type === 'vless' ? 'VLESS' : 'VMess';
      body.innerHTML = `
        <div class="kv"><div class="k">Type</div><div class="v">${{typeLabel}}</div></div>
        ...
        <div class="kv"><div class="k">Original Config</div><div class="v mono">${{esc(row.original || '')}}</div></div>
        <button type="button" class="copy-btn" data-config="${{enc(row.original)}}" onclick="copyConfig(this)">Copy Config</button>
      `;
    ...
    function render() {{
      ...
      const ty = typeEl.value || 'ALL';

      const filtered = data.filter((row) => rowMatches(row, q, c, i, k, t, ty));
      rowsEl.innerHTML = filtered.map((row, idx) => `
        <tr>
          <td data-label="#">${{idx + 1}}</td>
          <td data-label="Type"><span class="${{typeClass(row.type)}}">${{row.type ? row.type.toUpperCase() : '?'}}</span></td>
      ...
      const vmessCount = data.filter(r => r.type === 'vmess').length;
      const vlessCount = data.filter(r => r.type === 'vless').length;
      ...
      countVmessEl.textContent = `VMess: ${{vmessCount}}`;
      countVlessEl.textContent = `VLESS: ${{vlessCount}}`;
    }}
...
def process_vmess_content(
    ...
    """Process VMess content and return parsed results."""
    ...
    parse_progress = CliProgress("Parse VMess", len(tokens), enabled=show_progress)
...
def process_vless_content(
    ...
    """Process VLESS content and return parsed results."""
    tokens = extract_vless_tokens(content)
    if max_entries > 0:
        tokens = tokens[:max_entries]
    ...
    parse_progress = CliProgress("Parse VLESS", len(tokens), enabled=show_progress)
    for idx, token in enumerate(tokens, start=1):
        item = parse_vless(token)
        ...
        # Convert decoded object to compact json string
        if isinstance(item.get("decoded"), dict):
            item["decoded"] = json.dumps(
                item["decoded"], ensure_ascii=False, separators=(",", ":")
            )
...
def update_connectivity_status(
    ...
    """Update connectivity status for all rows using parallel workers."""
    candidates: List[Tuple[int, str, str, str, int, str]] = []
    for idx, row in enumerate(rows):
        ...
        proxy_type = str(row.get("type", "vmess")).strip()
        try:
            port = int(row.get("endpoint_port", 0) or 0)
        except Exception:
            port = 0
        candidates.append((idx, token, add, ip, port, proxy_type))
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        fut_map = {
            executor.submit(
                check_proxy_connectivity, proxy_type, token, add, ip, port, timeout
            ): idx
            for idx, token, add, ip, port, proxy_type in candidates
        }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Decode VMess/VLESS configs and generate HTML report",
...
Examples:
  ...
  # Check only VMess configs from local file
  python proxy_checker.py --mode vmess -i vmess_configs.txt

  # Check only VLESS configs from local file
  python proxy_checker.py --mode vless -i vless_configs.txt

  # Check all configs with custom output files
  python proxy_checker.py --mode all -o report.html --json data.json

  # Generate report from existing JSON without re-processing
  python proxy_checker.py --report-only --report-json data.json

  # Skip connectivity check, only parse and geolocate
  python proxy_checker.py --no-check-connectivity
  ...
  python proxy_checker.py --all
    ...
    parser.add_argument(
        ...
        "--input",
        default=None,
        help="Input config file (default: fetch from remote source based on mode)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=MODE_ALL,
        help="Proxy type to check: vmess, vless, or all (default: all)",
    )
    ...
    parser.add_argument(
        "--report-only",
        action="store_true",
        help="Generate HTML report from existing JSON without re-processing",
    )
    parser.add_argument(
        "--report-json",
        default="report.json",
    ...
    parser.add_argument(
        ...
        default=True,
        help="Check TCP connectivity and update JSON/HTML (default: enabled)",
    )
    parser.add_argument(
        "--connect-timeout",
        type=float,
        default=10.0,
        help="Max seconds per connectivity check (recommended: 10)",
    )
    parser.add_argument(
        "--connect-workers",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="How many entries to process (0 = all entries)",
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Process all entries in file",
    )
    parser.add_argument(
        "--no-progress",
        action="store_true",
    ...
    parser.add_argument(
        "--no-free-api",
        action="store_true",
        help="Disable fallback to free ipapi.co API",
    )
    ...
    source_url = DEFAULT_VMESS_URL
    if args.mode == MODE_VLESS:
        source_url = DEFAULT_VLESS_URL
    elif args.mode == MODE_ALL:
        # For 'all' mode, we'll handle both URLs
        pass
    ...
    if args.report_only:
        ...
        if args.mode == MODE_ALL:
            ...
            try:
                if args.input:
                    # If input file specified, try to read it as VMess
                    vmess_content, vmess_source = read_input_source(args.input, DEFAULT_VMESS_URL)
                else:
                    vmess_content, vmess_source = read_input_source(None, DEFAULT_VMESS_URL)
                print(f"VMess source: {vmess_source}")
            except Exception as e:
                print(f"Warning: Could not fetch VMess configs: {e}", file=sys.stderr)
            
            # Try to read VLESS
            try:
                if args.input:
                    # Try to find VLESS file (input file with 'vless' in name or .vless.txt)
                    vless_input = args.input.replace("vmess", "vless") if "vmess" in args.input else None
                    vless_content, vless_source = read_input_source(vless_input, DEFAULT_VLESS_URL)
                else:
                    vless_content, vless_source = read_input_source(None, DEFAULT_VLESS_URL)
                print(f"VLESS source: {vless_source}")
            except Exception as e:
                print(f"Warning: Could not fetch VLESS configs: {e}", file=sys.stderr)
            
            # Process VMess
            if vmess_content:
                vmess_rows = process_vmess_content(
                    vmess_content,
                    args.timeout,
                    api_keys,
                ...
                rows.extend(vmess_rows)
            
            # Process VLESS
            if vless_content:
                vless_rows = process_vless_content(
                    vless_content,
                    args.timeout,
                    api_keys,
                    show_progress=not args.no_progress,
                ...
                rows.extend(vless_rows)
            
            source_label = f"{vmess_source if vmess_content else 'none'} + {vless_source if vless_content else 'none'}"
            
        elif args.mode == MODE_VMESS:
            # Process only VMess
            content, source_label = read_input_source(args.input, DEFAULT_VMESS_URL)
            rows = process_vmess_content(
                content,
                args.timeout,
                api_keys,
                show_progress=not args.no_progress,
            ...
            # Process only VLESS
            content, source_label = read_input_source(args.input, DEFAULT_VLESS_URL)
            rows = process_vless_content(
                content,
                args.timeout,
                api_keys,
                show_progress=not args.no_progress,
    ...
    html_text = generate_html(rows, source_label, args.mode)
    with open(args.output, "w", encoding="utf-8") as hf:
        hf.write(html_text)

    # Print summary
    vmess_count = sum(1 for r in rows if r.get("type") == "vmess")
    vless_count = sum(1 for r in rows if r.get("type") == "vless")
